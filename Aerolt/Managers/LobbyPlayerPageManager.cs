...
namespace Aerolt.Managers
{
    public partial class LobbyPlayerPageManager : MonoBehaviour, IModuleStartup
    {
        ...
        private PlayerConfigBinding PlayerConfig
        {
            ...
            set
            {
                if (_playerConfig != null && _playerConfig != value) _playerConfig.UnBind(UpdateCheckboxValues);
                _playerConfig = value;
        ...
        private void MobSpawnsChanged()
        {
            ...
            foreach (var director in CombatDirector.instancesList)
                director.monsterSpawnTimer = disableMobSpawns.Value ? float.PositiveInfinity : 0f;
        }
        ...
        public void SetCurrency(CurrencyType currencyType, string strAmount)
        {
            ...
            new CurrencyMessage(master, currencyType, amount)
                .SendToServer(); // Send to server now just calls handle if we're on a server already
        }
