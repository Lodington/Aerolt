            ...
            Color itemColor, string itemName = "", string? outlinetext = null)
        {
            ...
            if (pos.z >= 0)
            {
                ...
                if (!string.IsNullOrEmpty(itemName))
                {
                    ...
                    GUI.color = Color.black;
                    GUI.Label(new Rect(pos.x - itemSize.x / 2 + 1, pos.y + 1 + size.y, itemSize.x, itemSize.y),
                        outlineItemContent);
                    GUI.Label(new Rect(pos.x - itemSize.x / 2 - 1, pos.y - 1 + size.y, itemSize.x, itemSize.y),
                        outlineItemContent);
                    GUI.Label(new Rect(pos.x - itemSize.x / 2 + 1, pos.y - 1 + size.y, itemSize.x, itemSize.y),
            ...
            if (pos.z >= 0)
            {
                ...
                for (var i = 0; i < itemNames.Count; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(itemName))
                    {
                        ...
                        GUI.color = Color.black;
                        GUI.Label(
                            new Rect(pos.x - itemSize.x / 2 + 1, pos.y + 1 + size.y + i * itemSize.y, itemSize.x,
                                itemSize.y), outlineItemContent);
                        GUI.Label(
                            new Rect(pos.x - itemSize.x / 2 - 1, pos.y - 1 + size.y + i * itemSize.y, itemSize.x,
                                itemSize.y), outlineItemContent);
                        GUI.Label(
                        ...
                        GUI.color = itemColor;
                        GUI.Label(
                            new Rect(pos.x - itemSize.x / 2, pos.y + size.y + i * itemSize.y, itemSize.x, itemSize.y),
                            itemContent);
                    }
                }
